...
def initialize_cachesingle():
    ...
    for l1 in range(SINGLE_MIN_LEN, SINGLE_MAX_LEN+1):
        cache_single.append([0] * (SINGLE_MAX_LEN+1))
        for l2 in range(SINGLE_MIN_LEN, SINGLE_MAX_LEN+1):
            if l1 == 0 and l2 == 0:
                continue
            elif l1 == 0:
                cache_single[-1][l2] += bulge_length[min(l2, BULGE_MAX_LEN)]
            elif l2 == 0:
                cache_single[-1][l2] += bulge_length[min(l1, BULGE_MAX_LEN)]
            else:
                # internal
                cache_single[-1][l2] += internal_length[min(l1+l2, INTERNAL_MAX_LEN)]
                # internal explicit
                if l1 <= EXPLICIT_MAX_LEN and l2 <= EXPLICIT_MAX_LEN:
                    idx = l1*EXPLICIT_MAX_LEN+l2 if l1<=l2 else l2*EXPLICIT_MAX_LEN+l1
                    cache_single[-1][l2] += internal_explicit[min(idx, EXPLICIT_MAX_LEN*EXPLICIT_MAX_LEN-1)]
                # internal symmetry
                if l1 == l2:
                    cache_single[-1][l2] += internal_symmetric_length[min(l1, SYMMETRIC_MAX_LEN)]
                else: # internal asymmetry
                    diff = abs(l1 - l2)
                    cache_single[-1][l2] += internal_asymmetry[min(diff, ASYMMETRY_MAX_LEN)]
...
def score_junction_A(i, j, nuci, nuci1, nucj_1, nucj, length):
    return (helix_closing_score(nuci, nucj) +
            (dangle_left_score(nuci, nuci1, nucj) if i < length - 1 else 0) +
            (dangle_right_score(nuci, nucj_1, nucj) if j > 0 else 0))
...
def score_hairpin(i, j, nuci, nuci1, nucj_1, nucj):
    return hairpin_length[min(j-i-1, HAIRPIN_MAX_LEN)] + score_junction_B(i, j, nuci, nuci1, nucj_1, nucj)
...
def score_single(i, j, p, q, length, nuci, nuci1, nucj_1, nucj, nucp_1, nucp, nucq, nucq1):
    l1 = p - i - 1
    l2 = j - q - 1
...
def score_multi(i, j, nuci, nuci1, nucj_1, nucj, length):
    return score_junction_A(i, j, nuci, nuci1, nucj_1, nucj, length) \
        + multi_paired + multi_base

def score_multi_unpaired(i, j):
    return (j-i+1) * multi_unpaired

def score_M1(i, j, k, nuci_1, nuci, nuck, nuck1, length):
    return score_junction_A(k, i, nuck, nuck1, nuci_1, nuci, length) \
...
def score_external_paired(i, j, nuci_1, nuci, nucj, nucj1, len):
    return score_junction_A(j, i, nucj, nucj1, nuci_1, nuci, len) + external_paired + base_pair_score(nuci, nucj)
